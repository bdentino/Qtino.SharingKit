...
@implementation NSObject (QtSharingSwizzle)

+ (void) load {
    NSLog(@"Loading QtSharingSwizzle");
}
...
+ (BOOL)qtsharing_swizzleClassMethod:(SEL)origSel_ withClassMethod:(SEL)altSel_ error:(NSError**)error_ {
    return [GetClass((id)self) qtsharing_swizzleMethod:origSel_ withMethod:altSel_ error:error_];
}
